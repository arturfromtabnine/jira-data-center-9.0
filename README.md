...
```bash
sudo su
echo "127.0.0.1  jira-cluster-900-lb" >> /etc/hosts
```
...
**(1) Start a Jira Data Center 9.0.0 Cluster**

```bash
rm -rf /opt/jira-cluster/9.0.0 | true
mkdir -p /opt/jira-cluster/9.0.0
mkdir -p /opt/jira-cluster/9.0.0/jira-home-shared
mkdir -p /opt/jira-cluster/9.0.0/jira-home-node1
mkdir -p /opt/jira-cluster/9.0.0/jira-home-node2
# If on linux fix permissions for volume mounts
# sudo chown 2001:2001 /opt/jira-cluster/9.0.0/jira-*
```
...
data-center/master/versions/9.0.0/docker-compose-one-node.yml"

docker-compose -f docker-compose-one-node.yml up --detach

# if some longer HTTP 503, try restarting Loadbalancer
docker-compose -f docker-compose-one-node.yml restart jira-cluster-900-lb
```
...
- Open a browser to [http://jira-cluster-900-lb:1900/](http://jira-cluster-900-lb:1900/)
...
4fb9ba30c689   codeclou/docker-atlassian-jira-data-center:loadbalancer-9837a52   "/work-private/docke…"   19 minutes ago   Up 18 minutes   0.0.0.0:1900->1900/tcp   jira-cluster-900-lb
6694a19445ae   atlassian/jira-software:9.0.0-jdk11                              "/usr/bin/tini -- /e…"   23 minutes ago   Up 23 minutes   8080/tcp                   jira-cluster-900-node1
c4253b09bce1   postgres:12.9                                                     "docker-entrypoint.s…"   23 minutes ago   Up 23 minutes   5432/tcp                   jira-cluster-900-db
```
...
Go to **[http://jira-cluster-900-lb:1900/](http://jira-cluster-900-lb:1900/)** and make sure you enabled cookies (sticky session).
...
Set the baseUrl to `http://jira-cluster-900-lb:1900` and continue.
...
```
cp -R /opt/jira-cluster/9.0.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/9.0.0/jira-home-node2
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/9.0.0/docker-compose-two-nodes.yml"

docker-compose -f docker-compose-two-nodes.yml up -d
docker-compose -f docker-compose-two-nodes.yml restart jira-cluster-900-lb
```
...
```bash
cd /opt/jira-cluster/9.0.0
mkdir -p /opt/jira-cluster/9.0.0/jira-home-node3
mkdir -p /opt/jira-cluster/9.0.0/jira-home-node4
# If on linux fix permissions for volume mounts
# sudo chown 2001:2001 /opt/jira-cluster/9.0.0/jira-*
```
...
```
cp -R /opt/jira-cluster/9.0.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/9.0.0/jira-home-node3
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/9.0.0/docker-compose-three-nodes.yml"

docker-compose -f docker-compose-three-nodes.yml up -d
docker-compose -f docker-compose-three-nodes.yml restart jira-cluster-900-lb
```
...
"https://raw.githubusercontent.com/codeclou/docker-atlassian-jira-\
data-center/master/versions/9.0.0/docker-compose-four-nodes.yml"

cp -R /opt/jira-cluster/9.0.0/jira-home-node1/{data,plugins,logos,import,export,caches} /opt/jira-cluster/9.0.0/jira-home-node4
docker-compose -f docker-compose-four-nodes.yml up -d
docker-compose -f docker-compose-four-nodes.yml restart jira-cluster-900-lb
```

To check call this multiple times, and it should output the different node ids after some time

```
curl -I -s http://jira-cluster-900-lb:1900 | grep X-ANODEID
```
...
#...
# DOCKER jira-cluster-900 network
